...
static constexpr uint32_t kMacGc64Magic = 0xB64CA110u;

void* mac_gc64_alloc(void* /*ud*/, void* ptr, size_t osize, size_t nsize) {
    if (nsize == 0) {
        if (ptr) {
            uint32_t magic = 0;
            memcpy(&magic, (char*)ptr - 8, 4);
            if (magic == kMacGc64Magic) {
                uint32_t off;
                memcpy(&off, (char*)ptr - 4, 4);
                free((char*)ptr - off);
            } else {
                free(ptr);  // legacy: allocated before mac_gc64_alloc was installed
            }
        }
        return nullptr;
    }

    bool isOurAlloc = false;
    uint32_t old_off = 8u;
    if (ptr) {
        uint32_t magic = 0;
        memcpy(&magic, (char*)ptr - 8, 4);
    ...
    if (isOurAlloc && new_off != old_off) {
        const size_t copy_size = (nsize < osize) ? nsize : osize;
        memmove(raw + new_off, raw + old_off, copy_size);
    } else if (!isOurAlloc && ptr) {
        // Legacy allocation: copy data into new GC64-safe block, then free old
        const size_t copy_size = (nsize < osize) ? nsize : osize;
        memcpy(raw + new_off, ptr, copy_size);
        free(ptr);
    }
    memcpy(raw + new_off - 8u, &kMacGc64Magic, 4u);  // sentinel at user[-8]
    memcpy(raw + new_off - 4u, &new_off, 4u);          // offset at user[-4]
    return raw + new_off;
}
